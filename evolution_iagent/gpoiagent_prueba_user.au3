...
if RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_BROWSER_EMULATION\", "iagent.exe") <> "9999" Then
	  ...
	  _FileWriteLog($logsroute,  @ComputerName & " ha agregado la clave 32 bits, usuario: " & _GetConsoleUser(), 1)
EndIf
...
If @OSArch = "X64" Then
   if RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_BROWSER_EMULATION\", "iagent.exe") <> "9999" Then
	  ...
	  _FileWriteLog($logsroute, @ComputerName & " ha agregado la clave 64 bits, usuario: " & _GetConsoleUser(), 1)
   EndIf
EndIf
...
If FileExists($Programfile & '\Evolution\Agente\iAgent.exe') Then
   ...
   $version = stringreplace(StringReplace(FileGetVersion($Programfile & '\Evolution\Agente\iAgent.exe',  $FV_PRODUCTVERSION), "BUILD", "")," ", ".")
	  ...
	  Else
		 ...
		 WEnd
	     FileMove($Programfile & '\Evolution\Agente\iAgent.exe', $programfile & '\Evolution\Agente\iAgentViejo.exe')
		 If @error = 1 then _FileWriteLog($logsroute , @ComputerName & " " & @OSArch & " fallo al mover el exe", 1)
	  If @OSArch = "X86" Then
   ...
   processWaitClose($iPID)
      ...
      $version2 = stringreplace(StringReplace(FileGetVersion($Programfile & '\Evolution\Agente\iAgent.exe',  $FV_PRODUCTVERSION), "BUILD", "")," ", ".")
		 if $version2 = $ultimaversion Then
			FileDelete($Programfile & '\Evolution\Agente\iAgentViejo.exe')
			_FileWriteLog($logsroute , @ComputerName & " " & @OSArch & " ha actualizado de " & $version & " a " & $version2, 1)
		 Else
			if FileExists($programfile & '\Evolution\Agente\iAgent.exe') Then
			   _FileWriteLog($logsroute , @ComputerName & " " & @OSArch & " ERROR, no se esperaba existencia de iagent.exe SE SALE, usuario: " & _GetConsoleUser(), 1)
			   Exit
			EndIf
			$i = 1
			while $i < 20
			   FileMove($Programfile & '\Evolution\Agente\iAgentViejo.exe', $programfile & '\Evolution\Agente\iAgent.exe')
			   if FileExists($Programfile & '\Evolution\Agente\iAgent.exe') then ExitLoop
			   Sleep(1000)
			   $i = $i + 1
			WEnd
			_FileWriteLog($logsroute , @ComputerName & " " & @OSArch & " fallo en la actualizacion de " & $version & " a " & $ultimaversion & " Usuario: " & _GetConsoleUser(), 1)
		 EndIf
  $a = 1
   while $a <10
...
Else
   ...
   processWaitClose($iPID)
      ...
      $version = stringreplace(StringReplace(FileGetVersion($Programfile & '\Evolution\Agente\iAgent.exe',  $FV_PRODUCTVERSION), "BUILD", "")," ", ".")
		 if $version = $ultimaversion Then
			_FileWriteLog($logsroute , @ComputerName & " " & @OSArch & " ha instalado iagent version " & $version & "usuario: " & _GetConsoleUser(), 1)
		 Else
			_FileWriteLog($logsroute , @ComputerName & " " & @OSArch & " fallo en la instalacion de iagent " & $version & " usuario: " & _GetConsoleUser(), 1)
		 EndIf
   FileDelete("C:\iagent_" & $ultimaversion & ".exe")
  $a = 1
   while $a < 10
...
if not FileExists(@ScriptDir & "\evolution.txt") Then
   _FileWriteLog($logsroute , @ComputerName & " " & @OSArch & " no existe evolution.txt, usuario: " & _GetConsoleUser(), 1)
   Exit
EndIf
...
While 1
	...
	if $eofile = -1 Then
		 ...
		 EndIf
		 _FileWriteLog($logsroute ,'No tiene configurado la extension en evolution.txt. No se hara ninguna modificacion en el puesto: ' & @ComputerName & "," & $puesto & "," & $serveraddress & " Usuario: " & _GetConsoleUser(), 1)
		 ExitLoop
    EndIf
	...
	if $separadocomas[1] = @ComputerName Then
	  ...
	  If $Puesto <> $SeparadoComas[2] Then
		 ...
		 EndIf
		 _FileWriteLog($logsroute , @ComputerName & " " & @OSArch & ' ha cambiado su extension de ' & $Puesto & ' a la extension ' & $SeparadoComas[2] & _GetConsoleUser(), 1)
	  EndIf
	  ...
	  If $ServerAddress <> $SeparadoComas[3] Then
		 ...
		 EndIf
		 _FileWriteLog($logsroute , @ComputerName & " " & @OSArch & ' ha cambiado su evoserver de ' & $serveraddress & ' al evoserver ' & $SeparadoComas[3] & " Usuario: " & _GetConsoleUser(), 1)
	  EndIf
   ExitLoop
   EndIf
WEnd
